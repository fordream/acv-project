...
public class PhysicsEditorShapeLibrary {
    ...
    private void append(Context context, String name, float pixelToMeterRatio) {  	
        ...
        try {
            ...
            if(TexturePackLoaderFromSource.isAsset){
            	parser.parse(context.getAssets().open(name), handler);
        	}else{
        		final File file = new File(TexturePackLoaderFromSource.pathSourceSD + name);
        		parser.parse(new InputSource(new BufferedInputStream(new FileInputStream(file))), handler);
        	}
        } catch (Exception e) {
            e.printStackTrace();
        }
