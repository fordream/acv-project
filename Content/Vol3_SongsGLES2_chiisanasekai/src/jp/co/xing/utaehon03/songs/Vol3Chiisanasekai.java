 ...
 */
public class Vol3Chiisanasekai extends BaseGameActivity implements
	...
	private TexturePackTextureRegionLibrary texpackLibBackGround,
			texpackLibItem, texpackLibVictory;
	...
	private float pPosition_GroundHide[][] = { { 518, 249 }, { 477, 35 },
			{ 746, 195 }, { 42, 155 }, { 294, 211 }, { 38, 216 }, { 602, 258 },
			{ 584, 380 }, { 787, 338 }, { 145, 215 }, { 321, 108 },
			{ 337, 316 }, { 388, 183 }, { 54, 393 }, { 81, 421 }, { 244, 45 },
			{ 188, 93 }, { 96, 54 }, { 619, 102 }, { 41, 0 }, { 380, -45 },
			{ 793, 74 }, { 497, 455 } };
	...
	private float pPosition_Rotate[][] = { { 115, 43 }, { 226, 15 },
			{ 373, 9 }, { 470, 17 }, { 610, 12 }, { 734, 12 } };
	...
	private float pPosition_Baloon[][] = { { 22, 273 }, { 26, 95 },
			{ 825, 283 }, { 840, 126 } };
	...
	/** POSITION HAND */
	private float arrpPosition_Hand[][] = { { 174, 49 }, { 285, 205 },
			{ 82, 330 }, { 108, 414 }, { 290, 305 }, { 400, 180 },
			{ 495, 120 }, { 420, 313 }, { 629, 255 }, { 627, 376 },
			{ 680, -4 }, { 795, 145 }, { 807, 187 }, { 745, 300 }, { 805, 334 } };

	/** POSITION FACE */
	private float arrpPosition_Face[][] = { { 171, 11 }, { 276, 170 },
			{ 74, 260 }, { 100, 345 }, { 283, 235 }, { 394, 110 },
			{ 487, 106 }, { 412, 243 }, { 622, 187 }, { 620, 307 },
			{ 673, -16 }, { 743, 73 }, { 799, 118 }, { 694, 226 }, { 797, 263 } };

	/** POSITION HOUSE */
	private float arrpPosition_House[] = { 802, 374, 481, 266, 588, 159, 695,
			52 };
	...
	private float arrPointtHouse[][] = { { 0, 54, 107, 107, 0, 0 },
			{ 60, 0, 60, 120, 120, 60 } };
	...
	public void onCreateResources() {
		...
		BitmapTextureAtlasTextureRegionFactory
				.setAssetBasePath("chiisanasekai/gfx/");
	...
	protected void loadKaraokeResources() {
		...
		texpackLibBackGround = texpackBackGround
		...
		// ttrBackGround
		ttrBackGround = texpackLibBackGround
				.get(Vol3ChiisanasekaiResource.background.A78_1_1_IPHONE_ID);

		// ttrHidingPlace
		for (int i = 0; i < ttrHidingPlace.length; i++) {
			ttrHidingPlace[i] = texpackLibBackGround
					.get(Vol3ChiisanasekaiResource.int_Hiding_Place[i]);
		}

		// VICTORY
		tttr_BGVictory = getTiledTextureFromPacker(texpackVictory,
				Vol3ChiisanasekaiResource.victory.A78_1_25_13_1_IPHONE_ID,
				Vol3ChiisanasekaiResource.victory.A78_1_25_13_2_IPHONE_ID);

		ttr_BGHand = texpackLibVictory
				.get(Vol3ChiisanasekaiResource.victory.A78_1_25_14_IPHONE_ID);

		for (int i = 0; i < ttrRotate.length; i++) {
			ttrRotate[i] = texpackLibVictory
					.get(Vol3ChiisanasekaiResource.int_Rotate[i]);
		}

		for (int i = 0; i < ttrBaloon.length; i++) {
			ttrBaloon[i] = texpackLibVictory
					.get(Vol3ChiisanasekaiResource.int_Baloon[i]);
		}

		for (int i = 0; i < ttrMove.length; i++) {
			ttrMove[i] = texpackLibVictory
					.get(Vol3ChiisanasekaiResource.int_Move[i]);
		}
		...
		for (int i = 0; i < ttrHand.length; i++) {
			ttrHand[i] = texpackLibItem
					.get(Vol3ChiisanasekaiResource.int_Hand[i]);
		}
		...
		for (int i = 0; i < tttr_Face_Move.length; i++) {
			...
			for (int j = 0; j < Vol3ChiisanasekaiResource.int_Face_Move[i].length; j++) {
				tmpFace_MOVE[j] = texpackLibItem
						.get(Vol3ChiisanasekaiResource.int_Face_Move[i][j]);
			}

			tttr_Face_Move[i] = new TiledTextureRegion(
					texpackItem.getTexture(), tmpFace_MOVE);
		}

		// tttr_Face
		for (int i = 0; i < tttr_Face.length; i++) {
			tttr_Face[i] = getTiledTextureFromPacker(texpackItem,
					Vol3ChiisanasekaiResource.int_Face[i]);
		}
		...
		for (int i = 0; i < tttrHouse.length; i++) {
			...
			for (int j = 0; j < Vol3ChiisanasekaiResource.int_House[i].length; j++) {
				tmpHouse[j] = texpackLibItem
						.get(Vol3ChiisanasekaiResource.int_House[i][j]);
			}

			tttrHouse[i] = new TiledTextureRegion(texpackItem.getTexture(),
					tmpHouse);
		}
	...
	protected void loadKaraokeSound() {
		...
		OGG_NAME_COLOR = new Sound[] { OGG_A78_5_1_2, OGG_A78_5_2_2,
				OGG_A78_5_3_2, OGG_A78_5_4_2, OGG_A78_5_5_2, OGG_A78_5_6_2,
				OGG_A78_5_7_2, OGG_A78_5_8_2 };
	...
	protected void loadKaraokeScene() {
		...
		if (sprBackGround == null)
			sprBackGround = new Sprite(0, 0, ttrBackGround,
					this.getVertexBufferObjectManager());
		mScene.attachChild(sprBackGround);

		// Hiding place
		for (int i = 0; i < sprHidingPlace.length; i++) {
			if (sprHidingPlace[i] == null)
				sprHidingPlace[i] = new Sprite(pPosition_GroundHide[i][0],
						pPosition_GroundHide[i][1], ttrHidingPlace[i],
						this.getVertexBufferObjectManager());
		...
		for (int i = 0; i < sprHand.length; i++) {
			if (sprHand[i] == null)
				sprHand[i] = new Sprite(0, 0, ttrHand[i],
						this.getVertexBufferObjectManager());
		...
		// Girl
		if (animsprGIRL == null)
			animsprGIRL = new AnimatedSprite(590, 278, tttrGIRL,
					this.getVertexBufferObjectManager()) {
				public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
					...
					case TouchEvent.ACTION_DOWN:
						if (checkContains(animsprGIRL, 70, 0, 243, 330, pGirlx,
								pGirly) && isTouchGIRL && !isMoveGIRL) {
							isTouchGIRL = false;
							isMoveGIRL = true;
							isMp3Girl = true;
					...
					case TouchEvent.ACTION_MOVE:
						...
						if (isMoveGIRL
								&& !isTouchGIRL
								&& Math.abs(((int) pSceneTouchEvent.getX() + (int) pSceneTouchEvent
										.getY()) - (lastGx + lastGy)) > 20) {
							if (isMp3Girl) {
								isMp3Girl = false;
							...
							if (animsprGIRL.getX() < -60) {
								animsprGIRL.setPosition(-60, pGirly
										- animsprGIRL.getHeight() / 2);
							} else if (animsprGIRL.getX() > 750) {
								animsprGIRL.setPosition(750, pGirly
										- animsprGIRL.getHeight() / 2);
							} else {
								animsprGIRL.setPosition(
										pGirlx - animsprGIRL.getWidth() / 2,
										pGirly - animsprGIRL.getHeight() / 2);
							}

							// If animsprGIRL is running
							if (!animsprGIRL.isAnimationRunning()) {
								animsprGIRL.animate(new long[] { 500, 500 }, 2,
										3, -1);
							}
					...
					case TouchEvent.ACTION_UP:
						if (!isTouchGIRL && isMoveGIRL) {
							...
							if (animsprGIRL.getY() >= animsprGIRL.getmYFirst()) {
								...
								animsprGIRL.setPosition(animsprGIRL.getX(),
										animsprGIRL.getmYFirst());
								animsprGIRL
		...
		animsprGIRL.setScaleCenter(animsprGIRL.getWidth() / 2,
				animsprGIRL.getHeight() / 2);
		...
		for (int i = 0; i < animspr_FACE.length; i++) {
			if (animspr_FACE[i] == null)
				animspr_FACE[i] = new AnimFace(0, 0, tttr_Face[i],
		...
		// Boy
		if (animsprBOY == null)
			animsprBOY = new AnimatedSprite(202, 358, tttrBOY,
					this.getVertexBufferObjectManager()) {
				public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
						float pTouchAreaLocalX, float pTouchAreaLocalY) {
					...
					switch (pSceneTouchEvent.getAction()) {
					case TouchEvent.ACTION_DOWN:
						if (checkContains(animsprBOY, 48, 5, 198, 233, pBoyx,
								pBoyy) && isTouchBOY && !isMoveBOY) {
					...
					case TouchEvent.ACTION_MOVE:
						...
						if (isMoveBOY
							...
							if (animsprBOY.getX() < -40) {
								animsprBOY.setPosition(-40,
										pBoyy - animsprBOY.getHeight() / 2);
							} else if (animsprBOY.getX() > 750) {
								animsprBOY.setPosition(750,
										pBoyy - animsprBOY.getHeight() / 2);
							} else {
								animsprBOY.setPosition(
										pBoyx - animsprBOY.getWidth() / 2,
										pBoyy - animsprBOY.getHeight() / 2);
							}

							// If animsprBoy is running
							if (!animsprBOY.isAnimationRunning()) {
								animsprBOY.animate(new long[] { 500, 500 }, 2,
										3, -1);
							}
					...
					case TouchEvent.ACTION_UP:
						if (!isTouchBOY && isMoveBOY) {
							...
							if (animsprBOY.getY() >= animsprBOY.getmYFirst()) {
								...
								animsprBOY.setPosition(animsprBOY.getX(),
										animsprBOY.getmYFirst());
								animsprBOY
										.registerEntityModifier(new DelayModifier(
												1.0f,
												new IEntityModifierListener() {
		...
		animsprBOY.setScaleCenter(animsprBOY.getWidth() / 2,
				animsprBOY.getHeight() / 2);
		animsprBOY.setScale(0.7f);
		animsprBOY.setZIndex(2);

		// Soldier
		if (animsprSoldier == null)
			animsprSoldier = new AnimatedSprite(790, 440, tttrSoldier,
					this.getVertexBufferObjectManager());
		...
		for (int i = 0; i < animsprHouse.length; i++) {
			if (animsprHouse[i] == null)
				animsprHouse[i] = new AnimHouse(arrpPosition_House[i], 515,
						tttrHouse[i], this.getVertexBufferObjectManager(), i);
		...
		for (int i = 0; i < animspr_FACE_MOVE.length; i++) {
			if (animspr_FACE_MOVE[i] == null)
				animspr_FACE_MOVE[i] = new AnimatedSprite(0, 0,
						tttr_Face_Move[i], this.getVertexBufferObjectManager());
		...
		if (animspr_BGVictory == null)
			animspr_BGVictory = new AnimatedSprite(123, 175, tttr_BGVictory,
					this.getVertexBufferObjectManager());
		...
		// Background hand
		if (spr_BGHand == null)
			spr_BGHand = new Sprite(140, 149, ttr_BGHand,
					this.getVertexBufferObjectManager());
		...
		for (int i = 0; i < sprRotate.length; i++) {
			if (sprRotate[i] == null)
				sprRotate[i] = new Sprite(pPosition_Rotate[i][0],
						pPosition_Rotate[i][1], ttrRotate[i],
						this.getVertexBufferObjectManager());
		...
		for (int i = 0; i < sprBaloon.length; i++) {
			if (sprBaloon[i] == null)
				sprBaloon[i] = new Sprite(pPosition_Baloon[i][0],
						pPosition_Baloon[i][1], ttrBaloon[i],
						this.getVertexBufferObjectManager());
		...
		for (int i = 0; i < sprMove.length; i++) {
			if (sprMove[i] == null)
				sprMove[i] = new Sprite(pPosition_Move[i][0],
						pPosition_Move[i][1], ttrMove[i],
						this.getVertexBufferObjectManager());
	...
	private void actionVictory() {
		...
		for (int i = 0; i < sprRotate.length; i++) {
			sprRotate[i].registerEntityModifier(new LoopEntityModifier(
					new SequenceEntityModifier(new RotationAtModifier(0.3f, 0,
							12, sprRotate[i].getWidth() / 2, sprRotate[i]
									.getHeight()), new RotationAtModifier(0.6f,
							12, -12, sprRotate[i].getWidth() / 2, sprRotate[i]
									.getHeight()), new RotationAtModifier(0.3f,
							-12, 0, sprRotate[i].getWidth() / 2, sprRotate[i]
									.getHeight())), -1));
		}

		for (int i = 0; i < sprBaloon.length; i++) {
			sprBaloon[i].registerEntityModifier(new LoopEntityModifier(
					new SequenceEntityModifier(new MoveYModifier(0.5f,
							sprBaloon[i].getY(), sprBaloon[i].getY() - 30),
							new MoveYModifier(0.5f, sprBaloon[i].getY() - 30,
									sprBaloon[i].getY())), -1));
		}
		// 10 270
		sprMove[1].registerEntityModifier(new LoopEntityModifier(
				new SequenceEntityModifier(new MoveModifier(0.5f, sprMove[1]
						.getX(), 10, sprMove[1].getY(), 270), new MoveModifier(
						0.5f, 10, sprMove[1].getX(), 270, sprMove[1].getY())),
				-1));

		// 753 271
		sprMove[0].registerEntityModifier(new LoopEntityModifier(
				new SequenceEntityModifier(new MoveModifier(0.5f, sprMove[0]
						.getX(), 753, sprMove[0].getY(), 271),
						new MoveModifier(0.5f, 753, sprMove[0].getX(), 271,
								sprMove[0].getY())), -1));
	}
	...
	private void visibleHand() {
		...
		for (int i = 0; i < sprHand.length; i++) {
			if (rdPositionHand[i] == 0) {
				sprHand[i].setPosition(arrpPosition_Hand[rdPositionHand[i]][0],
						arrpPosition_Hand[rdPositionHand[i]][1]);
				sprHand[i].setRotationCenter(0, 0);
				sprHand[i].setRotation(65);

				animspr_FACE[i].setPosition(
						arrpPosition_Face[rdPositionHand[i]][0],
						arrpPosition_Face[rdPositionHand[i]][1]);
				animspr_FACE[i].setRotationCenter(
						animspr_FACE[i].getWidth() / 2,
						animspr_FACE[i].getHeight() / 2);
				...
				sprHand[i].setPosition(arrpPosition_Hand[rdPositionHand[i]][0],
						arrpPosition_Hand[rdPositionHand[i]][1]);
				sprHand[i].setRotationCenter(sprHand[i].getWidth() / 2,
						sprHand[i].getHeight() / 2);
	...
	private void touchSoldier() {
		...
		animsprSoldier.registerEntityModifier(new MoveXModifier(5.5f,
				animsprSoldier.getX(), 0 - animsprSoldier.getWidth(),
				new IEntityModifierListener() {
					@Override
					...
					public void onModifierFinished(IModifier<IEntity> arg0,
							IEntity arg1) {
						animsprSoldier.setPosition(960,
								animsprSoldier.getmYFirst());
						animsprSoldier
								.registerEntityModifier(new MoveXModifier(1.0f,
										960, animsprSoldier.getmXFirst(),
										new IEntityModifierListener() {
											@Override
											public void onModifierStarted(
													IModifier<IEntity> arg0,
	...
	private void checkFinishCount() {
		...
		if (finish_COUNT == 8) {
			...
			animsprBOY.registerEntityModifier(new DelayModifier(1.5f,
					new IEntityModifierListener() {
						@Override
						public void onModifierStarted(IModifier<IEntity> arg0,
								IEntity arg1) {
	 ...
	 * CHECK GAME FINISH STATE
	 * */
	private void checkFinishCount1() {
		linkedAnimFACE.clear();
		// mScene.unregisterUpdateHandler(timerFace);
		...
		animsprBOY.registerEntityModifier(new DelayModifier(0.2f,
				new IEntityModifierListener() {
					@Override
					public void onModifierStarted(IModifier<IEntity> arg0,
							IEntity arg1) {
					...
					public void onModifierFinished(IModifier<IEntity> arg0,
						...
						// victoryVisible(fals);
						// OGG_A78_1_OMEDETO.play();
						spr_BGHand.registerEntityModifier(new DelayModifier(
								0.2f, new IEntityModifierListener() {
									@Override
									public void onModifierStarted(
											IModifier<IEntity> arg0,
											IEntity arg1) {
									...
									public void onModifierFinished(
										...
										animsprBOY.setVisible(true);
										animsprBOY.setPosition(
												animsprBOY.getmXFirst(),
												animsprBOY.getmYFirst());

										animsprGIRL.setVisible(true);
										animsprGIRL.setPosition(
												animsprGIRL.getmXFirst(),
												animsprGIRL.getmYFirst());

										animsprSoldier.setVisible(true);
										animsprSoldier.setPosition(
												animsprSoldier.getmXFirst(),
												animsprSoldier.getmYFirst());
			...
			public void onTimePassed(TimerHandler arg0) {
				if (linkedAnimFACE.size() != 0) {
					...
					runOnUpdateThread(new Runnable() {
						@Override
						public void run() {
							linkedAnimFACE.get(display).registerEntityModifier(
	...
	public void resetALL() {
		...
		if (animsprSoldier != null) {
			...
			animsprSoldier.stopAnimation();
			animsprSoldier.setCurrentTileIndex(0);
			animsprSoldier.setPosition(animsprSoldier.getmXFirst(),
					animsprSoldier.getmYFirst());
		}
		...
		if (animsprBOY != null) {
			...
			animsprBOY.stopAnimation();
			animsprBOY.setCurrentTileIndex(0);
			animsprBOY.setPosition(animsprBOY.getmXFirst(),
					animsprBOY.getmYFirst());
		}
		...
		if (animsprGIRL != null) {
			...
			animsprGIRL.stopAnimation();
			animsprGIRL.setCurrentTileIndex(0);
			animsprGIRL.setPosition(animsprGIRL.getmXFirst(),
					animsprGIRL.getmYFirst());
		}
	...
	private class AnimFace extends AnimatedSprite {
		...
		public AnimFace(float pX, float pY,
				ITiledTextureRegion pTiledTextureRegion,
				VertexBufferObjectManager vertexBufferObjectManager, int fIndex) {
		...
		@Override
		public boolean onAreaTouched(TouchEvent pTouchAreEvent,
				float pTouchAreaLocalX, float pTouchAreaLocalY) {
					...
					// if this Face visible = true
						...
						// if face in house
							...
							thisFace.registerEntityModifier(new SequenceEntityModifier(
									new ScaleAtModifier(0.4f, 1.0f, 1.4f, 31,
											78), new ScaleAtModifier(0.4f,
											1.4f, 1.0f, 31, 78,
											new IEntityModifierListener() {
												@Override
												public void onModifierStarted(
			...
			case TouchEvent.ACTION_MOVE:
				if (face_MOVE
						&& !face_DOWN
						&& !check
						&& Math.abs(((int) pTouchAreEvent.getX() + (int) pTouchAreEvent
								.getY()) - (animfaceX + animfaceY)) > 10) {
					face_SE_MOVE = true;
					face_UP = true;
					...
					thisFace.setPosition(faceX - thisFace.getWidth() / 2 - 25,
							faceY - thisFace.getHeight() / 2 + 5);
					sprHand[thisFace.fIndex].setVisible(false);

					animspr_FACE_MOVE[thisFace.fIndex].setVisible(true);

					if (!animspr_FACE_MOVE[thisFace.fIndex]
							.isAnimationRunning()) {
						animspr_FACE_MOVE[thisFace.fIndex].animate(new long[] {
								400, 400 }, 0, 1, -1);
					}
					animspr_FACE_MOVE[thisFace.fIndex].setPosition(faceX
							- thisFace.getWidth() / 2 - 70,
							faceY - thisFace.getHeight() / 2);
			...
			case TouchEvent.ACTION_UP:
				// face move
				if (face_MOVE && !face_DOWN && !check && face_UP) {
					face_MOVE = false;
					face_UP = false;
					// check face collidesWith house
					for (int i = 0; i < animsprHouse.length; i++) {
						// collidesWith house
						if (checkContainsPolygon(animsprHouse[i],
								arrPointtHouse, 5, faceX, faceY)
							...
							if (thisFace.fIndex == animsprHouse[i].hIndex) {
								...
								animspr_FACE_MOVE[thisFace.fIndex]
										.setVisible(false);
								animspr_FACE_MOVE[thisFace.fIndex]
										.stopAnimation();

								animsprHouse[thisFace.fIndex]
										.setCurrentTileIndex(1);

								thisFace.registerEntityModifier(new DelayModifier(
				...
				// face don't move
					...
					if (face_NO_HOUSE && !check) {
						check = true;
						thisFace.registerEntityModifier(new DelayModifier(1.0f,
								new IEntityModifierListener() {
									@Override
									public void onModifierStarted(
											IModifier<IEntity> arg0,
		...
		private void CaseElse() {
			...
			thisFace.setPosition(
					arrpPosition_Face[rdPositionHand[thisFace.fIndex]][0],
	...
	private class AnimHouse extends AnimatedSprite {
		...
		public AnimHouse(float pX, float pY,
				ITiledTextureRegion pTiledTextureRegion,
				VertexBufferObjectManager vertexBufferObjectManager, int hIndex) {
			super(pX, pY, pTiledTextureRegion, vertexBufferObjectManager);
		...
		public boolean onAreaTouched(TouchEvent pTouchAreEvent,
			...
			case TouchEvent.ACTION_DOWN:
				...
				if (thisHouse.getCurrentTileIndex() == 0 && house_DOWN) {
					house_DOWN = false;
					...
					thisHouse.registerEntityModifier(new LoopEntityModifier(
							new SequenceEntityModifier(new MoveXModifier(0.05f,
									thisHouse.getmXFirst(), thisHouse
											.getmXFirst() - 3),
									new MoveXModifier(0.1f, thisHouse
											.getmXFirst() - 3, thisHouse
											.getmXFirst() + 3),
									new MoveXModifier(0.05f, thisHouse
											.getmXFirst() + 3, thisHouse
											.getmXFirst())), 3));
					...
					if (!animspr_FACE[thisHouse.hIndex].isVisible()) {
						...
						thisHouse.registerEntityModifier(new DelayModifier(
								1.3f, new IEntityModifierListener() {
									@Override
									public void onModifierStarted(
											IModifier<IEntity> arg0,
											IEntity arg1) {
						...
						animspr_FACE[thisHouse.hIndex]
								.registerEntityModifier(new SequenceEntityModifier(
										new ScaleAtModifier(0.4f, 1.0f, 1.4f,
												31, 78), new ScaleAtModifier(
												0.4f, 1.4f, 1.0f, 31, 78),
	...
	private void timerSoundDelay(final Sound pSound, final float timeDelay) {
		mScene.registerUpdateHandler(new TimerHandler(timeDelay,
				new ITimerCallback() {
