...
public class Vol3Koinobori extends BaseGameActivity implements
	...
	private TexturePack Vol3KoinoboriPacker_1, Vol3KoinoboriPacker_2,
			Vol3KoinoboriPacker_3, Vol3KoinoboriPacker_4;
	private TexturePackTextureRegionLibrary Vol3KoinoboriPackerLibrary1,
			Vol3KoinoboriPackerLibrary2, Vol3KoinoboriPackerLibrary3,
			Vol3KoinoboriPackerLibrary4;
	...
	public void loadKaraokeResources() {
		...
		this.tiledTtrRRedBoy = getTiledTextureFromPacker(
		...
		// Load YellowBoy
		this.tiledTtrRYellowBoy = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_2,
		...
		// Load RedBear
		this.tiledTtrRBear = getTiledTextureFromPacker(
		...
		// Load Cat
		this.tiledTtrRCat = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
		...
		// Load Kumo
		this.tiledTtrRKumo = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
		...
		// Load Mogura
		this.tiledTtrRMogura = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
		...
		// Load Tanpopo
		this.tiledTtrRTanpopoOne = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
		...
		this.tiledTtrRTanpopoTwo = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
		...
		this.tiledTtrRTanpopoFour = getTiledTextureFromPacker(
				Vol3KoinoboriPacker_3,
	...
	public void loadKaraokeScene() {
		...
		addGimmicsButton(mScene, Vol3KoinoboriResource.SOUND_GIMMIC,
				Vol3KoinoboriResource.IMAGE_GIMMIC, Vol3KoinoboriPackerLibrary3);
	...
	public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {
		...
		if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_DOWN) {
			// || Polygon.contains(pX, pY, pxPolygonTree, pyPolygonTree, 4)
			if (this.sprYellowBall.contains(pX, pY)) {
				//if (isResetTree) {
					mp3Syu2.play();
					resetTree();
				//}
			} else if (this.sprYaguruma.contains(pX, pY)
	...
	class MySprite extends Sprite {
		...
		public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
			...
			case TouchEvent.ACTION_MOVE:
				 ...
				 */
				if (instance != null) {
					instance.setPosition(pX - this.getWidth() * 2 / 3, pY
							- this.getHeight() / 2);
				}
				break;
					...
					if (index <= 4) {
						...
						if (Polygon.contains(
							...
							arrAnmSprFlag[countFlagOnTree] = new FlagAnimatedSprite(
									0, 0, arrTiledTtrRFlag[this.index],
									this.index, mVertexBufferObjectManager);
							...
							if (!anmSprRedBoy.isOnTree()) {
								...
								if (!anmSprRedBoy.isAnimationRunning()
									...
									if (!anmSprRedBoy.isHasHat()) {
										long durations[] = { 400, 400, 400,
												400, 400, 400, 50 };
										int frames[] = { 1, 2, 1, 2, 1, 2, 0 };
										anmSprRedBoy.animate(durations, frames,
								...
								if (!anmSprYellowBoy.isAnimationRunning()
									...
									if (!anmSprYellowBoy.isHasHat()) {
										long durations[] = { 400, 400, 400,
												400, 400, 400, 50 };
										int frames[] = { 1, 2, 1, 2, 1, 2, 0 };
										anmSprYellowBoy.animate(durations,
												frames, 0,
								...
								if (!anmSprBear.isAnimationRunning()
									...
									if (!anmSprBear.isHasHat()) {
										long durations[] = { 400, 400, 400,
												400, 400, 400, 50 };
										int frames[] = { 1, 2, 1, 2, 1, 2, 0 };
										anmSprBear.animate(durations, frames,
												0, new IAnimationListener() {
					 ...
					 */
						...
						if (checkContains(
							...
							if (!anmSprRedBoy.isAnimationRunning()
								...
								mp3Powa.play();
								if (anmSprRedBoy.isOnTree()) {
									// Check If Red Boy on Tree Contains
									// Instance
